...
namespace NzbDrone.Core.Applications.Readarr
{
    public class Readarr : ApplicationBase<ReadarrSettings>
    {
            ...
            catch (HttpException ex)
            {
                switch (ex.Response.StatusCode)
                {
                    ...
                    case HttpStatusCode.BadRequest:
                        ...
                        failures.AddIfNotNull(new ValidationFailure("FetcharrUrl", "Fetcharr URL is invalid, Readarr cannot connect to Fetcharr"));
                        break;
                    case HttpStatusCode.SeeOther:
                        _logger.Warn(ex, "Readarr returned redirect and is invalid");
                        failures.AddIfNotNull(new ValidationFailure("BaseUrl", "Readarr URL is invalid, Fetcharr cannot connect to Readarr - are you missing a URL base?"));
                        break;
                    default:
        ...
        private ReadarrIndexer BuildReadarrIndexer(IndexerDefinition indexer, DownloadProtocol protocol, int id = 0)
        {
            ...
            readarrIndexer.Fields.FirstOrDefault(x => x.Name == "baseUrl").Value = $"{Settings.FetcharrUrl.TrimEnd('/')}/{indexer.Id}/";
