...
namespace NzbDrone.Core.Applications.Whisparr
{
    public class Whisparr : ApplicationBase<WhisparrSettings>
    {
            ...
            catch (HttpException ex)
            {
                switch (ex.Response.StatusCode)
                {
                    ...
                    case HttpStatusCode.BadRequest:
                        ...
                        failures.AddIfNotNull(new ValidationFailure("FetcharrUrl", "Fetcharr URL is invalid, Whisparr cannot connect to Fetcharr"));
                        break;
                    case HttpStatusCode.SeeOther:
                        _logger.Warn(ex, "Whisparr returned redirect and is invalid");
                        failures.AddIfNotNull(new ValidationFailure("BaseUrl", "Whisparr URL is invalid, Fetcharr cannot connect to Whisparr - are you missing a URL base?"));
                        break;
                    default:
        ...
        private WhisparrIndexer BuildWhisparrIndexer(IndexerDefinition indexer, DownloadProtocol protocol, int id = 0)
        {
            ...
            whisparrIndexer.Fields.FirstOrDefault(x => x.Name == "baseUrl").Value = $"{Settings.FetcharrUrl.TrimEnd('/')}/{indexer.Id}/";
